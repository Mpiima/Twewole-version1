...
<div class="col-12">
            <div class="card">
              ...
              if(isset($_POST['savedata']) && $type=="benefit"){
                ...
                $pid=$_POST['id'];
                  //insert benefites
                $insert_pdetail=$dbh->query("INSERT INTO p_details(productid,benefits,benefit_title,type)
                values('$pid','$description','$title','benefit')");
                if($insert_pdetail){echo "<div class='alert alert-success'>Added</div>";}else{ echo "<div class='alert alert-danger'>failed</div>";}            
              }else if(isset($_POST['savedata']) && $type=="feature"){
                $title=$_POST['title'];
                $description=$_POST['description'];
                $pid=$_POST['id'];
                $insert_pdetails=$dbh->query("INSERT INTO p_details(productid,feature,feature_title,type)
                values('$pid','$description','$title','feature')");
                 if($insert_pdetails){echo "<div class='alert alert-success'>Added</div>";}else{ echo "<div class='alert alert-danger'>faile</div>";}
              }
               if(isset($_POST['delete'])){
                $del = $_POST['did'];
                $dele=$dbh->query("DELETE  FROM p_details where autoid=$del");
                if($dele){
              ...
              if(isset($_POST['edit_data'])){
                ...
                if($ty=="benefit"){
                  $update=$dbh->query("UPDATE p_details SET benefits='$description', benefit_title='$title' WHERE autoid=$autoid ");
                }else if($ty=="feature"){
                  $update=$dbh->query("UPDATE p_details SET feature='$description', feature_title='$title' WHERE autoid=$autoid ");
                  
                }    
                if($update){echo "<div class='alert alert-success'>Edited</div>";}else{ echo "<div class='alert alert-danger'>Failed</div>";}
