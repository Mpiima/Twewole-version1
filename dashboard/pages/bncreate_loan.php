...
if (isset($_POST['saveloan'])) {
    ...
    if (empty($summary)) {
        ...
        $stmt_insert = $dbh->prepare("INSERT INTO products (addedby, type, nature, institution, amount_range, summary, link, status,
         title, loan_id, advert,accountTypeNumber) 
            ...
            <div class="row">
                ...
                <div class="row">
                  <div class="col-sm-4">
                    ...
                    <input required required type="file" name="fileToUpload" id="fileToUpload" class="form-control" >
                    </div>
                    </div>
                  </div>
                  ...
                  <div class="row">
                    ...
                    <div class="card-body">
                        <textarea  required ="true" class="form-control rounded-0" id="exampleFormControlTextarea2" name="summary">
                       </textarea> 
                    </div>
                    </div>
                  </div>
              ...
              <div class="card-body">
                ...
                cellspacing="0" width="100%" role="grid" aria-describedby="example_info"
                 style="width: 100%;">
                  ...
                  <tbody>
                    <?php 
                    // $result_products=$dbh->query("SELECT * FROM products where status=1 AND type='loan' AND addedby='".$_SESSION['rolenumber']."' order by autodate desc ");
                   
                    $result_products=$dbh->query("SELECT * FROM products INNER JOIN users ON products.addedby = users.rolenumber
                    where products.status=1 AND products.type='loan' AND products.addedby = '".$_SESSION['rolenumber']."'  ORDER BY products.auto_id desc ");
